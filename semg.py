...
frequency_domain_features = dataset.iloc[:, 1:9].apply(lambda x: haar_wavelet_transform(butter_bandpass_filter(x.values, lowcut=20, highcut=500, fs=4000)), axis=1) 

# Remove rows with empty arrays (signals that caused issues) 
frequency_domain_features = frequency_domain_features[frequency_domain_features.apply(len) > 0] 

window_size = 10  # You can adjust the window size as needed 
processed_features = frequency_domain_features.apply(lambda x: moving_average(x, window_size)) 

# Combine processed features 
X = np.vstack(processed_features.values) 
y = dataset.loc[frequency_domain_features.index, 'class'].values 
